"""Evaluador SPF (RFC 7208, función check_host) — RA1-04.

El flujo n8n LEE el resultado SPF que declara la cabecera Authentication-Results. Este módulo lo
RECALCULA: toma la IP de origen confiable y el registro SPF publicado, y decide.

Cobertura: `ip4`, `ip6`, `a`, `mx`, `include`, `exists`, `all`, modificador `redirect`, límite de
10 términos con consulta DNS (§4.6.4). Fuera de alcance, declarado:
- macros (`%{i}` ...): resultado `no_soportado`, que el agente trata como "no verificable";
- `ptr`: cuenta como consulta y nunca coincide (la RFC lo desaconseja);
- límite de consultas vacías (§4.6.4, void lookups).
"""
