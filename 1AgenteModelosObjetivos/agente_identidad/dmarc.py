"""DMARC recalculado (RFC 7489) — RA1-06.

DMARC es la pregunta de identidad correcta: ¿el dominio del From visible —el que ve el usuario—
está respaldado por un SPF o un DKIM que pasan Y están alineados con ese dominio?

Regla de rigor: sólo se concluye `fail` cuando ninguna vía pudo haber pasado. Si el mensaje tiene
firma DKIM y no tenemos el crudo para verificarla, podría existir una firma válida: el resultado es
`no_evaluable`, no `fail`.
"""
