...
Registro honesto de lo que se probó y lo que falta, a completar con la corrida de la VPS.
...
| `verificar_e2e.sh` completo | ✘ **pendiente**. La primera corrida encontró dos errores de los scripts, exclusivos de Git Bash (salida CRLF de `jq.exe` y conversión de rutas para `curl.exe`); ambos se corrigieron. Queda por correr completa |
