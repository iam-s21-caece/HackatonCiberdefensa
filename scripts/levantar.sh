...
# Un GET a un webhook POST publicado responde "not registered for GET"; uno sin publicar, "not registered".
for puerta in analizar whatsapp sms; do
  respuesta="$(curl -s "$(url_n8n)/webhook/centinela/$puerta" || true)"
  if printf '%s' "$respuesta" | grep -q 'GET'; then ok "webhook POST $(url_n8n)/webhook/centinela/$puerta"
  else fallar "el webhook centinela/$puerta no está publicado: docker compose logs n8n-importar n8n"; fi
done
