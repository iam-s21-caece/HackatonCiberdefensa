"""Crítico: compara las decisiones con la verdad humana bajo un estándar de costos — RA2-03.

Convención explícita (es la que corrige el uso de VERDADERO/FALSO_POSITIVO como veredictos):
los veredictos del flujo son ACCIONES (bloquear / escalar / entregar); TP/FP/FN/TN sólo existen al
compararlas con el veredicto humano. Se reportan dos lecturas:
- detección: positivo = NO entregar (bloquear o escalar). Mide si algo malicioso llegó al usuario.
- bloqueo automático: positivo = bloquear. Mide cuánto resuelve el sistema sin un humano.

El estándar de desempeño es externo al agente y configurable: entregar un malicioso (FN) cuesta
mucho más que demorar un legítimo (FP), y escalar cuesta tiempo de analista.
"""
