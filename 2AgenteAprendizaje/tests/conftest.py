"""Fixtures del Agente 2.

`fixtures/reports/*.json` son reportes REALES del flujo generados con
`herramientas/generar_reportes.py`: 4 muestras × {flujo original, flujo con Agente 1} × IA
{coincide, dice "legítimo", caída}. `fixtures/escenarios.json` dice, por reporte, qué combinación
es y la verdad conocida de la muestra. Las respuestas de la IA están simuladas por el harness.
"""
